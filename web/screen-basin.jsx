...
function TimeMachine({ lang }) {
  ...
  return (
    <div className="hud-panel" style={{ padding: 14 }}>
      ...
      {/* scene — REAL HLS imagery of the Azraq core, per year */}
      <div style={{ position: "relative", aspectRatio: "16/9", borderRadius: "var(--r-md)", overflow: "hidden", border: "1px solid var(--line)", direction: "ltr" }}>
        <YearImagery year={Math.min(year, 2025)} bbox={[31.70, 36.55, 32.10, 37.15]} />
        {/* year badge */}
        <div style={{ position: "absolute", top: 10, insetInlineStart: 10, fontFamily: "var(--font-num)", fontWeight: 800, fontSize: 22, color: "#fff", background: "rgba(7,16,31,.7)", padding: "4px 12px", borderRadius: 8, border: "1px solid var(--line-strong)" }} className="num">{Math.min(year, 2025)}</div>
        <div style={{ position: "absolute", bottom: 10, insetInlineStart: 10, fontSize: 10.5, color: "#cbd5e1", background: "rgba(7,16,31,.7)", padding: "3px 9px", borderRadius: 999, border: "1px solid var(--line-strong)" }}>{t.tm_real}</div>
  ...
  return (
    <div style={{ height: "100%", overflowY: "auto", padding: "var(--s6)" }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 18, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}><RegionBadge lang={lang} /><DataBadge lang={lang} /></div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--s5)", alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--s5)" }}>
          <div className="hud-panel" style={{ padding: 0, overflow: "hidden", position: "relative", height: 300 }}>
            <LiveMap lang={lang} mode="basin" layers={{ basins: true, heat: true, dots: true, sites: false }} nasaLayer="ndvi" />
