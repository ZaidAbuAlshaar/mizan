...
function ScreenMap({ lang, onNav, loading }) {
  ...
  const [layers, setLayers] = useState({ basins: true, heat: true, dots: true, sites: false });
  const [nasa, setNasa] = useState("none");
  const toggle = k => setLayers(s => ({ ...s, [k]: !s[k] }));
  const REAL = D2.__real;
  const flagsCount = REAL ? D2.fields.filter(f => (f.score || 0) >= 70).length : 23;
  const m3Low = REAL ? (REAL.est_m3_low / 1e6).toFixed(1) : 4;
  const m3High = REAL ? (REAL.est_m3_high / 1e6).toFixed(1) : 6;
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "366px 1fr", height: "100%", gap: 0 }}>
        ...
        <div style={{ display: "grid", gap: "var(--s3)" }}>
          <div className="animate-rise" style={{ animationDelay: ".05s" }}>
            <StatTile icon="flag" label={t.s1_redflags} sub={t.s1_redflags_sub} accent="var(--danger)" demo lang={lang}
              valueNode={<div className="stat-big" style={{ fontSize: 40, color: "var(--danger)" }}>{D(flagsCount, lang)}</div>} />
          </div>
          <div className="animate-rise" style={{ animationDelay: ".1s" }}>
            <StatTile icon="drop" label={t.s1_m3} sub={t.s1_m3_sub} demo lang={lang}
              valueNode={<div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                <span className="stat-big" style={{ fontSize: 38 }}>{D(m3Low, lang)}–{D(m3High, lang)}</span>
      ...
      <div style={{ position: "relative", overflow: "hidden", background: "radial-gradient(900px 700px at 60% 40%, rgba(15,30,52,.6), var(--bg-deep))" }}>
        <LiveMap lang={lang} layers={layers} nasaLayer={nasa}
          onBasin={b => onNav("basin", { basin: b.id })}
          onField={f => onNav("queue", { focus: f.rank })} />

        {/* top overlay: region badge + data badge */}
        ...
        {/* NASA live layer picker */}
        <div style={{ position: "absolute", top: 16, insetInlineEnd: 16, width: 200 }}>
          <NasaLayerPicker lang={lang} value={nasa} onChange={setNasa} />
        </div>
        ...
        <div className="hud-panel" style={{ position: "absolute", bottom: 16, insetInlineStart: 16, width: 230, padding: 0 }}>
          ...
          <div style={{ padding: "0 8px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
            ...
            <LayerChip active={layers.dots} onClick={() => toggle("dots")} color="var(--accent)">{D2.__real ? t.candidates_layer : t.s1_candidates}</LayerChip>
            <LayerChip active={layers.sites} onClick={() => toggle("sites")} color="var(--accent-2)">{t.layer_sites}</LayerChip>
          </div>
