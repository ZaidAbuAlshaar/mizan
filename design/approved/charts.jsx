...
function GraceChart({ active, lang, height = 300, showIrrigated, showDA }) {
  ...
  const y = mkScale(3, -21, m.t, m.t + ih);
  ...
  const area = `${line} L${x(last.y)},${y(-21)} L${x(obs[0].y)},${y(-21)} Z`;
  const [hover, setHover] = useState(null);

  const yticks = [3, -3, -9, -15, -21];
  ...
  return (
    <div style={{ position: "relative", direction: "ltr" }}>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}
        ...
        {/* gap connector dotted */}
        <line x1={x(2016)} y1={y(-8.3)} x2={x(2019)} y2={y(-9.9)} stroke="var(--text-3)" strokeDasharray="2 4" strokeWidth="1.5" />
...
/* ---------------- NDVI chart ---------------- */
function NdviChart({ active, lang, height = 170 }) {
  const data = window.MIZAN_DATA.ndvi;
  const W = 460, H = height;
  const m = { t: 14, r: 14, b: 26, l: 30 };
  const iw = W - m.l - m.r, ih = H - m.t - m.b;
  const x = mkScale(2016, 2026, m.l, m.l + iw);
  const y = mkScale(0, 0.8, m.t + ih, m.t);
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 7, gap: 10 }}>
        <span style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "var(--font-head)", fontWeight: 700, fontSize: 14 }}>
          {label}{demo && <DemoBadge lang={lang} small />}
        </span>
